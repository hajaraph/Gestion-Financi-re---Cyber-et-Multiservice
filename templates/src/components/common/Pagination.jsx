...
/**
 * Reusable Modern Pagination Component with Items Per Page selector
 * 
 ...
 * @param {function} onPerPageChange - Callback when items per page is changed
...
const Pagination = ({
    ...
    return (
        <div className={`flex flex-col items-center justify-between gap-6 py-8 mt-4 border-t border-gray-100 ${className}`}>
            <div className="flex flex-col sm:flex-row items-center gap-4 w-full justify-between px-4">
                {/* Items Per Page Selector */}
                <div className="flex items-center gap-2 text-sm text-gray-500 font-medium bg-white px-3 py-1.5 rounded-xl border border-gray-100 shadow-sm animate-in fade-in slide-in-from-left-2">
                    <span>Afficher</span>
                    <select
                        value={itemsPerPage}
                        onChange={(e) => onPerPageChange && onPerPageChange(Number(e.target.value))}
                        className="bg-gray-50 border border-gray-200 rounded-lg px-2 py-1 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all cursor-pointer hover:border-blue-300 font-bold text-blue-600"
                    >
                ...
                {/* Summary Text */}
                <div className="text-sm text-gray-500 font-medium bg-gray-50 px-4 py-1.5 rounded-full border border-gray-100 shadow-sm animate-in fade-in slide-in-from-right-2">
            ...
            {/* Pagination Controls */}
            {totalPages > 1 && (
                <nav className="flex items-center space-x-2 animate-in fade-in zoom-in-95 duration-500" aria-label="Pagination">
                    {/* Previous Button */}
                    <button
